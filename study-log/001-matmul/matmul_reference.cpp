 ...
 * or Apple Accelerate) provide heavily optimized, assembly-level implementations
 * of matrix operations. We use CBLAS `dgemm` (Double-precision General Matrix Multiply)
 * to establish the golden standard for both correctness (reference output) and
...
void reference(const double *A, const double *B, double *C, int M, int N,
  ...
  // - lda, ldb, ldc: Leading dimensions. For row-major, this is the number of columns.
