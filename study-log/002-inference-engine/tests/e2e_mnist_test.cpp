...
// ── M2: General DAG support (mnist_ffn_complex.onnx with Add) ────────────────

TEST(E2EMnistComplexTest, ModelLoadsSuccessfully) {
  CpuAllocator alloc;
  ModelData data = OnnxLoader::Load(kComplexModelPath, &alloc);
...
TEST(E2EMnistComplexTest, EngineBuildsSuccessfully) {
  CpuAllocator alloc;
  ModelData data = OnnxLoader::Load(kComplexModelPath, &alloc);
...
TEST(E2EMnistComplexTest, InferenceProducesValidOutput) {
  CpuAllocator alloc;
  ModelData model = OnnxLoader::Load(kComplexModelPath, &alloc);
  ...
  std::cout << "[E2E Complex] Prediction for image_0: " << predicted << "\n";
}
